...
s3=$(printf "%05d" $s2)
schi=$(curl --connect-timeout $timeout -s "http://www.aastocks.com/tc/stocks/quote/frame-quote.aspx?symbol=$s3" -H "Referer: http://www.aastocks.com/tc/stocks/quote/quick-quote.aspx?symbol=$s3&S=Y" |  sed -e 's/var/\'$'\nvar/g' | grep "^var \_Quote" | cut -d',' -f1 | cut -d':' -f2 | sed 's/"//g')
